...
//
// Data model:
//   nodes:   id -> { id, name, states[], parents[], cpt[], description?, aiSources? }
...
//
// `aiSources` is an optional array of citations attached by the AI enrichment
// feature — each { title, url, excerpt?, polarity: 'positive'|'negative',
// weight: 0..1, affectsState? }. Persisted so users can revisit why a node's
// probabilities were set the way they were. Omitted from JSON when empty.
//
...
export class BayesNet {
  ...
  addNode({ id, name, states, parents = [], cpt = null, description, aiSources, x, y }) {
    ...
    if (Array.isArray(aiSources) && aiSources.length) {
      node.aiSources = aiSources.map(normalizeSource);
    }
  ...
  setAiSources(id, sources) {
    const n = this.getNode(id);
    if (!Array.isArray(sources) || sources.length === 0) {
      delete n.aiSources;
    } else {
      n.aiSources = sources.map(normalizeSource);
    }
  }
  ...
  toJSON() {
    ...
    for (const id of this.topologicalOrder()) {
      ...
      if (n.aiSources && n.aiSources.length) serialized.aiSources = n.aiSources.map(s => ({ ...s }));
...
function normalizeSource(s) {
  ...
  if (s?.excerpt) out.excerpt = String(s.excerpt).slice(0, 500);
  if (s?.highlight) out.highlight = String(s.highlight).slice(0, 200);
  if (s?.affectsState) out.affectsState = String(s.affectsState);
  return out;
}
